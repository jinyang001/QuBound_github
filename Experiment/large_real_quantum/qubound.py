...
# After training
if best_model_state:
    torch.save(best_model_state, 'best_model_state_GHZ12_test.pth')
...
for x in x_values:
    ...
    plt.plot(date, predicted_upper_Z, linestyle='--', label='Predicted Upper Bound_Z', color='red')
    plt.plot(date, predicted_lower_Z, linestyle='--', label='Predicted Lower Bound_Z', color='green')
    plt.plot(date, actual_Z, label='Test Actual_Z', color='blue')
    ...
    data_to_save = pd.DataFrame({
        ...
        'Predicted Lower Bound_Z': predicted_lower_Z.flatten(),
        'Predicted Upper Bound_Z': predicted_upper_Z.flatten()
    })

    # Write the DataFrame to a CSV file
    data_to_save.to_csv('output_12_test.csv', index=False)
