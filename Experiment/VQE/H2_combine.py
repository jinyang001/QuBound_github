...
def get_top_states_from_circuit(qc, num_shots=10000, num_top=8):
...
CARE_STATES = [format(i, '02b') for i in range(2 ** 2)]

print(CARE_STATES)
temp = get_top_states_from_circuit(qc)
...
# data = pd.read_csv('datasets/ibmq_mumbai/H2_IZ_ibmq_mumbai.csv')
data = pd.read_csv('../../datasets/ibmq_mumbai/H2_XX_ibmq_mumbai.csv')
# Removing outliers
...
date_test = np.array([datetime.strptime(date, '%Y-%m-%d %H:%M:%S') for date in date_test])
...
# # After training
# if best_model_state:
#     torch.save(best_model_state, 'best_model_state_XX.pth')
...
# model.load_state_dict(torch.load('best_model_state_IZ.pth'))
model.eval()
...
#     predicted_upper_Z1 = np.zeros(predicted_upper_bounds.shape[0])
#     predicted_lower_Z1 = np.zeros(predicted_lower_bounds.shape[0])
#     actual_Z1 = np.zeros(actual_values.shape[0])
#
...
#         predicted_upper_Z1[i] = np.sum(
#             coeffs1 * predicted_upper_bounds[i, :] * (coeffs1 > 0) + coeffs1 * predicted_lower_bounds[i, :] * (
#                         coeffs1 < 0))
#         predicted_lower_Z1[i] = np.sum(
#             coeffs1 * predicted_lower_bounds[i, :] * (coeffs1 > 0) + coeffs1 * predicted_upper_bounds[i, :] * (
#                         coeffs1 < 0))
#         actual_Z1[i] = np.sum(coeffs1 * actual_values[i, :])
#
#     predicted_upper_Z1 = predicted_upper_Z1 * -1.052373245772859
#     predicted_lower_Z1 = predicted_lower_Z1 * -1.052373245772859
#     actual_Z1 = actual_Z1 * -1.052373245772859
#     print(actual_Z1[0])
#     print(predicted_upper_Z1[0])
#     print(predicted_lower_Z1[0])
#     print('sssss')
#
#     predicted_upper_Z2 = np.zeros(predicted_upper_bounds.shape[0])
#     predicted_lower_Z2 = np.zeros(predicted_lower_bounds.shape[0])
#     actual_Z2 = np.zeros(actual_values.shape[0])
...
#     for i in range(predicted_upper_bounds.shape[0]):
#         # For each state, determine whether to add or subtract its probability bound
#         predicted_upper_Z2[i] = np.sum(
#             coeffs2 * predicted_upper_bounds[i, :] * (coeffs2 > 0) + coeffs2 * predicted_lower_bounds[i, :] * (
#                         coeffs2 < 0))
#         predicted_lower_Z2[i] = np.sum(
#             coeffs2 * predicted_lower_bounds[i, :] * (coeffs2 > 0) + coeffs2 * predicted_upper_bounds[i, :] * (
#                         coeffs2 < 0))
#         actual_Z2[i] = np.sum(coeffs2 * actual_values[i, :])
#
#     predicted_upper_Z2 = predicted_upper_Z2 * 0.39793742484318045
#     predicted_lower_Z2 = predicted_lower_Z2 * 0.39793742484318045
#     actual_Z2 = actual_Z2 * 0.39793742484318045
#     print(actual_Z2[0])
#     print(predicted_upper_Z2[0])
#     print(predicted_lower_Z2[0])
#     print('sssss')
#
#     predicted_upper_Z3 = np.zeros(predicted_upper_bounds.shape[0])
#     predicted_lower_Z3 = np.zeros(predicted_lower_bounds.shape[0])
#     actual_Z3 = np.zeros(actual_values.shape[0])
#
#     coeffs3 = np.array([1, -1, 1, -1])
#
#     # Calculate expectation bounds and actual for each sample
#
#     for i in range(predicted_upper_bounds.shape[0]):
#         # For each state, determine whether to add or subtract its probability bound
#         predicted_upper_Z3[i] = np.sum(
#             coeffs3 * predicted_upper_bounds[i, :] * (coeffs3 > 0) + coeffs3 * predicted_lower_bounds[i, :] * (
#                         coeffs3 < 0))
#         predicted_lower_Z3[i] = np.sum(
#             coeffs3 * predicted_lower_bounds[i, :] * (coeffs3 > 0) + coeffs3 * predicted_upper_bounds[i, :] * (
#                         coeffs3 < 0))
#         actual_Z3[i] = np.sum(coeffs3 * actual_values[i, :])
#
#     predicted_upper_Z3 = predicted_upper_Z3 * -0.39793742484318045
#     predicted_lower_Z3 = predicted_lower_Z3 * -0.39793742484318045
#     actual_Z3 = actual_Z3 * -0.39793742484318045
#     print(actual_Z3[0])
#     print(predicted_upper_Z3[0])
#     print(predicted_lower_Z3[0])
#     print('sssss')
#
#     predicted_upper_Z4 = np.zeros(predicted_upper_bounds.shape[0])
#     predicted_lower_Z4 = np.zeros(predicted_lower_bounds.shape[0])
#     actual_Z4 = np.zeros(actual_values.shape[0])
#     coeffs4 = np.array([1, -1, -1, 1])
#
#     # Calculate expectation bounds and actual for each sample
#
#     for i in range(predicted_upper_bounds.shape[0]):
#         # For each state, determine whether to add or subtract its probability bound
#         predicted_upper_Z4[i] = np.sum(
#             coeffs4 * predicted_upper_bounds[i, :] * (coeffs4 > 0) + coeffs4 * predicted_lower_bounds[i, :] * (
#                         coeffs4 < 0))
#         predicted_lower_Z4[i] = np.sum(
#             coeffs4 * predicted_lower_bounds[i, :] * (coeffs4 > 0) + coeffs4 * predicted_upper_bounds[i, :] * (
#                         coeffs4 < 0))
#         actual_Z4[i] = np.sum(coeffs4 * actual_values[i, :])
#
#     predicted_upper_Z4 = predicted_upper_Z4 * -0.01128010425623538
#     predicted_lower_Z4 = predicted_lower_Z4 * -0.01128010425623538
#     actual_Z4 = actual_Z4 * -0.01128010425623538
#     print(actual_Z4[0])
#     print(predicted_upper_Z4[0])
#     print(predicted_lower_Z4[0])
#     print('sssss')
#
#     # predicted_upper_Z=predicted_upper_Z1+predicted_upper_Z2+predicted_upper_Z3+predicted_upper_Z4
#     # predicted_lower_Z=predicted_lower_Z1+predicted_lower_Z2+predicted_lower_Z3+predicted_lower_Z4
#
#     predicted_upper_Z = predicted_lower_Z1 + predicted_upper_Z2 + predicted_lower_Z3 + predicted_lower_Z4
#     predicted_lower_Z = predicted_upper_Z1 + predicted_lower_Z2 + predicted_upper_Z3 + predicted_upper_Z4
#     actual_Z = actual_Z1 + actual_Z2 + actual_Z3 + actual_Z4
#     print(actual_Z[0])
#     print(predicted_upper_Z[0])
#     print(predicted_lower_Z[0])
#     print('sssss')
#
...
#         # For each state, determine whether to add or subtract its probability bound
#         predicted_upper_Z1[i] = np.sum(
#             coeffs1 * predicted_upper_bounds[i, :] * (coeffs1 > 0) + coeffs1 * predicted_lower_bounds[i, :] * (coeffs1 < 0))
#         predicted_lower_Z1[i] = np.sum(
#             coeffs1 * predicted_lower_bounds[i, :] * (coeffs1 > 0) + coeffs1 * predicted_upper_bounds[i, :] * (coeffs1 < 0))
#         actual_Z1[i] = np.sum(coeffs1 * actual_values[i, :])
#
#     predicted_upper_Z1 = predicted_upper_Z1 * 0.18093119978423156
#     predicted_lower_Z1 = predicted_lower_Z1 * 0.18093119978423156
#     actual_Z1 = actual_Z1 * 0.18093119978423156
#     print(actual_Z1[0])
#     print(predicted_upper_Z1[0])
#     print(predicted_lower_Z1[0])
#     print('sssss')
#
#     return predicted_upper_Z1.reshape(-1, 1), predicted_lower_Z1.reshape(-1, 1), actual_Z1.reshape(-1, 1)
...
for x in x_values:
    ...
    #     predicted_upper_bound_selected, predicted_lower_bound_selected, actual_selected, qc.num_qubits
    # )


    def combine_expectation_values(file_Z, file_X):
        # Load the data from the Z observables
        data_Z = np.load(file_Z)
        predicted_upper_Z = data_Z['predicted_upper_Z']
        predicted_lower_Z = data_Z['predicted_lower_Z']
        actual_Z = data_Z['actual_Z']

        # Load the data from the X observables
        data_X = np.load(file_X)
        predicted_upper_X = data_X['predicted_upper_X']
        predicted_lower_X = data_X['predicted_lower_X']
        actual_X = data_X['actual_X']

        # Combine the Z and X data
        combined_upper = predicted_upper_Z + predicted_upper_X
        combined_lower = predicted_lower_Z + predicted_lower_X
        combined_actual = actual_Z + actual_X

        return combined_upper, combined_lower, combined_actual


    predicted_upper_Z, predicted_lower_Z, actual_Z = combine_expectation_values('z_observables.npz', 'x_observables.npz')
    ...
    plt.plot(date_test, predicted_upper_Z, linestyle='--', label='Predicted Upper Bound', color='red')
    plt.plot(date_test, predicted_lower_Z, linestyle='--', label='Predicted Lower Bound', color='green')
    plt.plot(date_test, actual_Z, label='Test Actual', color='blue')
    plt.axhline(y=-1.5588536262512207, color='orange', linestyle='-', label='True Expectation Value w/o noise')

    plt.xlabel('Sample')
    plt.ylabel('Expectation Value')
    start_date = min(date_test)
    end_date = max(date_test)
    ...
    # plt.legend()
    plt.ylim(-1.6, -1.42)
    plt.yticks(np.arange(-1.6, -1.42, 0.02))
    plt.xticks(rotation=45)
    plt.title(f'Test Data VQE-EffSU2-Observable: Actual vs Predicted with Bounds (Confidence: {confidence_level}%)')
    plt.savefig('case_study1.eps', format='eps', dpi=1000)
    plt.tight_layout()
    plt.show()
    ...
    statewise_bcr_Z, statewise_bdp_Z = calculate_bcr_bdp_for_each_state(
        predicted_upper_Z, predicted_lower_Z, actual_Z
    )
    # After calculating the bounds for Z observables
    # np.savez('z_observables.npz', predicted_upper_Z=predicted_upper_Z, predicted_lower_Z=predicted_lower_Z,
    #          actual_Z=actual_Z)

    # # After calculating the bounds for X observables
    # np.savez('x_observables.npz', predicted_upper_X=predicted_upper_Z, predicted_lower_X=predicted_lower_Z,
    #          actual_X=actual_Z)
