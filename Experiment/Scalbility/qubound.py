...
# CARE_STATES = get_top_states_from_circuit(qc)
CARE_STATES=['000','111']
# CARE_STATES=['000000','111111']
# CARE_STATES=['000000000','111111111']
# CARE_STATES=['000000000000','111111111111']
# CARE_STATES=['000000000000000','111111111111111']
print(CARE_STATES)
...
# # After training
# if best_model_state:
#     torch.save(best_model_state, 'best_model_state_GHZ3.pth')
...
model.load_state_dict(torch.load('best_model_state_GHZ3.pth'))
model.eval()
...
for x in x_values:
    ...
    plt.tight_layout()
    plt.show()
    print(predicted_upper_bound_selected.shape)
    # sss

    plt.figure(figsize=(10, 6))
    plt.grid(axis='y', ls="--")
    plt.scatter(new_date_test, actual_selected[:, 0].reshape(-1, 1), color='black', label='Target Values (ZZZ Observable)')
    plt.plot(new_date_test, predicted_upper_bound_selected[:, 0].reshape(-1, 1), label='Upper Bounds', linestyle='--', color='red')
    plt.plot(new_date_test, predicted_lower_bound_selected[:, 0].reshape(-1, 1), label='Lower Bounds', linestyle='--', color='green')
    ...
    # plt.savefig('GHZ15_000.eps', format='eps', dpi=1000)
    plt.show()
    ...
    for idx, (ax, state) in enumerate(zip(axes.ravel(), states_to_plot)):
        ...
        ax.plot(new_date_test,predicted_upper_bound_selected[:, idx], linestyle='--', label='Predicted Upper Bound', color='red')
        ax.plot(new_date_test,predicted_lower_bound_selected[:, idx], linestyle='--', label='Predicted Lower Bound', color='green')
        # ax.plot(predicted_selected[:, idx], label=f'Test Predicted {state}', color='orange')
        ax.plot(new_date_test,actual_selected[:, idx], label=f'Test Actual {state}', color='blue')
        ax.set_xlabel('Date')
        ax.set_ylabel('Probability Bounds')
        # ax.legend()
    ...
    # plt.plot(predicted_upper_bound_selected[:, 0], linestyle='--', label='Predicted Upper Bound', color='blue')
    # plt.plot(predicted_lower_bound_selected[:, 0], linestyle='--', label='Predicted Lower Bound', color='black')
    # plt.plot(actual_selected[:, 0], 'o', label=f'Test Actual {state}', color='blue')
    # plt.plot(predicted_upper_bound_selected[:, 1], linestyle='--', label='Predicted Upper Bound', color='red')
    # plt.plot(predicted_lower_bound_selected[:, 1], linestyle='--', label='Predicted Lower Bound', color='green')
    # plt.plot(actual_selected[:, 1], 'o', label=f'Test Actual {state}', color='red')
    # plt.xlabel('Sample')
    # plt.ylabel('Value')
    # plt.legend()
